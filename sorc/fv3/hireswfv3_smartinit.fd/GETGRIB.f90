...
!  TYPE(ISET), INTENT(INOUT) :: iprcp
   INTEGER, INTENT(INOUT) :: ISNOW(:,:),IZR(:,:),IIP(:,:),IRAIN(:,:)
        ...
        REAL, allocatable,dimension(:,:) :: HTAGL
...
! Need for hourly Guam ARW output - so move to this section of code
! visibility 
       JPDT(1) = 19
       JPDT(2) = 000
       JPDT(10) = 1
...
!mptest       J=K
         htagl=hght(:,:,ll)-zs
         write(6,*) 'min/max of HTAGL: ',LL, minval(Htagl), maxval(Htagl)
      ENDDO
...
! Need sfc wind gust for hourly GUAM ARW so move to this portion of code
!  sfc wind gust 
